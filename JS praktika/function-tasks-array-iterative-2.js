...
{
  function isIT(person) {
    return person.faculty === 'Informatikos fakultetas';
  }
}
const ifstudents = students.filter(isIT)

console.log(ifstudents);
...
console.groupCollapsed('5. Atspausdinti visų Informatikos fakulteto studentų vidurkius');
{
  function mapStudentsWithAvgIf(students) {
    ...
    for (let i = 0; i < ifstudents.length; i++) {
      ...
      studentsWithAveragesIf.push({
        ...student,
        avg: moduleAveragesWithWeights / allCredits
      })
    }
    return studentsWithAveragesIf;
  }

  console.table(mapStudentsWithAvgIf(students))
}
  console.groupEnd();
  ...
  console.groupCollapsed('6. Atspausdinti visų Chemijos fakulteto studentų vidurkius');
  {
      ...
      function mapStudentsWithAvgCh(students) {
        const studentsWithAveragesCh = [];
        for (let i = 0; i < chem_students.length; i++) {
          const student = chem_students[i];
          let allCredits = 0;
          let moduleAveragesWithWeights = 0;
          for (let x = 0; x < student.modules.length; x++) {
            const module = student.modules[x];
          ...
          studentsWithAveragesCh.push({
            ...student,
            avg: moduleAveragesWithWeights / allCredits
          })
        }
        return studentsWithAveragesCh;
      }
  
      console.table(mapStudentsWithAvgCh(students))
  ...
  console.groupCollapsed('7. Atspausdinti visų Elektros ir elektronikos fakulteto studentų vidurkius');
  {
    ...
    function mapStudentsWithAvgEl(students) {
      const studentsWithAveragesEl = [];
      for (let i = 0; i < Elstudents.length; i++) {
        const student = Elstudents[i];
        let allCredits = 0;
        let moduleAveragesWithWeights = 0;
        for (let x = 0; x < student.modules.length; x++) {
          const module = student.modules[x];
        ...
        studentsWithAveragesEl.push({
          ...student,
          avg: moduleAveragesWithWeights / allCredits
        })
      }
      return studentsWithAveragesEl;
    }

    console.table(mapStudentsWithAvgEl(students))
  ...
  {
    function firstCourse(person) {
      return person.course === 1;
    }

    const firststudents = students.filter(firstCourse)
  ...
  {
    function secondCourse(person) {
      return person.course === 2;
    }

    const secondstudents = students.filter(secondCourse)
  ...
  {
    function thirdCourse(person) {
      return person.course === 3;
    }

    const thirdstudents = students.filter(thirdCourse)

    console.log(thirdstudents);
  ...
  {
    function fourthCourse(person) {
      return person.course === 4;
    }

    const fourthstudents = students.filter(fourthCourse)

    console.log(fourthstudents);
  }
  console.groupEnd();
