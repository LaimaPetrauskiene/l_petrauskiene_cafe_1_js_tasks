...
{
  people.forEach(p => console.log(`${p.sex} ${p.income}`));
}
console.groupEnd();

console.groupCollapsed('5. Sukurtite masyvą su vardais, pavardėmis ir lytimi, pagal pradinį žmonių masyvą');
{
  people.forEach(p => console.log(`${p.name} ${p.surname} ${p.sex}`));
}
...
{
  const menOnly = people.filter(p => p.sex === 'male');
  console.log(menOnly);
}
console.groupEnd();
...
{
  const womenOnly = people.filter(p => p.sex === 'female');
...
{
  const hasCars = people.filter(p => p.hasCar === true);
  hasCars.forEach(p => console.log(`${p.name} ${p.surname}`));
}
console.groupEnd();

console.groupCollapsed('9. Atspausdinkite žmones kurie yra susituokę');
{
  const areMarried = people.filter(p => p.married === true);
  areMarried.forEach(p => console.log(`${p.name} ${p.surname}`));
}
console.groupEnd();
...
const newPropertiesName = people.map( item => {
  const { income: salary, ...rest } = item;
  return { salary, ...rest }
 }
);

console.log( newPropertiesName );
}
console.groupEnd();
